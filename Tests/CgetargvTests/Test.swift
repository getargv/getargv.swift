...
struct CgetargvTests {
    ...
    func GetArgvOfPidNuls(nuls: Bool, separator: String) throws {
        ...
        let expectedOutput = try #require(ProcessInfo.processInfo.arguments.joined(separator: separator).data(using: .nonLossyASCII))

        let actualOutput = Data(
            buffer: UnsafeBufferPointer<CChar>(start: res.start_pointer!, count: res.end_pointer - res.start_pointer + 1)
        )
